...
def algorithm_contest():
    ...
    for i, k in enumerate(KEYS):
        ...
        ax.text(i + (j - 1) * w, v + 0.012, "*", ha="center", fontsize=13, color="#b3261e")
    ...
    ax.set_title("Three-algorithm contest per disease  (* = selected)", pad=12)
...
def triage_accuracy():
    ...
    ax.set_title(f"Triage recovery from partial information\n{sm['n_conditions']} conditions, "
                 f"{sm['n_symptoms']} symptoms", pad=12, fontsize=10.5)
...
def llm_benchmark():
    ...
    a1.set_title("Task-specific benchmark", fontsize=10.5); a1.invert_yaxis()
    ...
    a2.set_title("Safety failures (lower is better)", fontsize=10.5)
...
def triage_sources():
    ...
    ax.set_title(f"Provenance of the {sm['n_conditions']} triage conditions", pad=14, fontsize=10.5)
...
def architecture():
    ...
    def box(x, y, w, h, text, fc="#ffffff", ec=SLATE, fs=9, bold=False, tc=SLATE):
    ...
    def arrow(x1, y1, x2, y2, label=None):
        ...
        if label:
            ax.text((x1 + x2) / 2, (y1 + y2) / 2 + 0.18, label, ha="center",
                    fontsize=7.5, color="#5c7683", style="italic")
    ...
    for label, x in models:
        box(x, 4.7, 1.76, 1.0, label, fc="#ffffff", fs=8)
    ...
    box(0.6, 1.2, 4.0, 1.1, "Local language model\ngemma3:4b over loopback\n(observed features only)",
        fc="#ffffff", ec=TEAL, fs=8.5)
    box(5.4, 1.2, 4.0, 1.1, "Deterministic prescribing\nIndian OTC formulary\n(chosen in code)",
        fc="#ffffff", ec=TEAL, fs=8.5)
    arrow(3.6, 3.0, 2.6, 2.3)
    arrow(6.4, 3.0, 7.4, 2.3)

    box(2.6, 0.0, 4.8, 0.85, "Patient-facing note and prescription",
        fc="#e6f4f3", ec=TEAL, bold=True, tc=TEAL)
    arrow(2.6, 1.2, 4.2, 0.85)
    arrow(7.4, 1.2, 5.8, 0.85)

    ax.text(5.0, 9.85, "IHACDP request path", ha="center", fontsize=12, fontweight="bold", color=DARK if False else SLATE)
    fig.tight_layout(); fig.savefig(OUT / "fig_architecture.png", dpi=200, bbox_inches="tight")
    plt.close(fig)



if __name__ == "__main__":
    for fn in (architecture, roc_curves, algorithm_contest, metrics_panel, triage_accuracy, llm_benchmark, triage_sources):
